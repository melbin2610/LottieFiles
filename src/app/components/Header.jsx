        ...
        <nav className=" text-base text-[#20272c] karla_regular flex gap-5 max-xl:hidden font-normal">
          ...
          <div className="flex gap-5 items-center">
            ...
            <div className="flex items-center gap-3 max-xl:justify-end">
              {/* Search Icon */}
             {!isSearchOpen&& <div className="hidden max-xl:block cursor-pointer" onClick={toggleSearchMenu}>
                <FaSearch className="text-black" />
              </div>}

              {/* Login Link */}
              <Link href="/" className="text-[#019d91] text-[16px] font-bold cursor-pointer hover:text-green-200 max-sm:hidden karla_bold">
                Login
              </Link>
            ...
            {/* Mobile Menu Button */}
           {isSearchOpen ? <button className="xl:hidden" onClick={toggleSearchMenu}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={true ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16m-7 6h7"} />
              </svg>
            </button>: <button className="xl:hidden" onClick={toggleMobileMenu}>
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      {isSearchOpen && (
        <div className="inset-0 z-20 flex flex-col justify-between bg-white p-4 items-center h-[89vh]">
          ...
          <div className="flex justify-between items-center w-full max-w-md sm:hidden">
            ...
            <div className="bg-[#019d91] text-white py-2 px-4 rounded-[9px] w-24 font-bold cursor-pointer hover:bg-green-200">Sign Up</div>
          </div>
        </div>
      )}
    </div>
