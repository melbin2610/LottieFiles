...
const ResourceCard = ({ image, title, readTime, id, slug }) => (
  <Link
    href={`/${slug}`}
    className="w-[270px] max-md:w-[100%] max-sm:w-[100%]  bg-[#101417] mt-6 "
  >
    <Image
      className="w-[100%] h-[135px] max-md:h-auto max-sm:h-auto rounded-xl "
      src={image}
      alt={title}
    />
    <h2 className="text-[20px] text-[#ffff] font-bold mt-3  max-xl:text-[16px]  arboria_bold">
      {title}
    </h2>
    <p className="text-[16px] text-[#d6fef1b3] mt-3 karla_regular max-sl:mt-[2px]">
      {readTime}
    </p>
  </Link>
);
